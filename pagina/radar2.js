...
const commandMap = {
	measures: {
		jump: (value) =>{
			...
			rd.hzre = roundArrTo(getFieldIfExists(value,'hzre'), 2);
			...
			console.log('rd.hzre ', rd.hzre);
			console.log('rd.hzth ', rd.hzth);
			console.log('rd.vz0 ', rd.vz0);
			console.log('rd.vert ', rd.vert);
	...
	state: {
		...
		jmpcalibr: (value) => {
			console.log('jmpcalibr receive');
			setElem(currBoardId, "jmpcalibr", '', '');
		},
		ntarget: (value) => {
...
function createBoardSection(boardID) {
	...
	<div class='col-1 col-s-12' id='radarmode-${boardID}'>
		...
		<div class='col-1 col-s-12' id='areacalibr-${boardID}'>
			<div class="txt"><p>Salto target</p></div>
			<select name="target" class="sel button-large">
				<option value="1">Target 1</option>
				<option value="2">Target 2</option>
				<option value="3">Target 3</option>
			</select>
			<input class="send button-small button-blue" type="button" value="Calibra"/>
		</div>
		<div class='col-1 col-s-12' id='areavertices-${boardID}'>
			<div class="txt"><p>Stato</p><br></div>
			<div id='connstate-${boardID}'>
				<div id='iotstate-${boardID}'><input class="iotmsg button-text" type="text" value="IoT OFF"/></div>
...
function setInputListeners(boardID) {
	...
	/// RADAR TARGET JUMP CALIBRATION ///////////////////////////////////////////////////////////////////////////////////////
	let radarcalibr = document.getElementById(`jmpcalibr-${boardID}`);
	let radarcalibrsend = radarcalibr.querySelector('.send');
	radarcalibrsend.onclick = () => {
		let targetsel = radarcalibr.querySelector('.sel');
		let target = targetsel.value;
		pubAtt("jmpcalibr", target, boardId, "write"); //serializza e invia
		radarcalibr.style.backgroundColor = "#E67E22"; // activate the wait signal for command feedback
	}
}
